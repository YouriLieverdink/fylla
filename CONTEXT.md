...
A per-client goal of **hours to deliver each month**, met by the whole team assigned to that client — all developers' logged hours **plus the manager's own project hours** count toward it. This is **team-aggregate**, and orthogonal to personal utilization (the same manager hours count toward both). A Fylla-owned figure, **effective-dated** (ADR-0017): a baseline default plus target changes recorded as "from month M onward, the target is H", each persisting forward until the next change; resolving a month picks the latest change effective on or before it, falling back to the default. Not auto-scaled for holidays.
...
**Delivery projection**:
A forecast of where a managed client's monthly delivery is heading: the hours **Delivered** so far this month, extended at the current run-rate to month-end, read against the **Client monthly target** to flag whether the month will land **over or under** the agreed hours. Run-rate = delivered ÷ working days elapsed, scaled to the month's working days (Mon–Fri, not adjusted for holidays — as the target isn't). Purely derived from synced worklogs and the target; no sprint, estimate, or plan-line data. Shown per managed client on the Delivery view.
_Avoid_: sprint pacing (an earlier framing that split the target across sprints; dropped — more machinery than the monthly billing question needs)
